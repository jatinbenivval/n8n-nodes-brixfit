...
class Brixfit {
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let requestOptions = { method: 'GET', url: '', headers, json: true };
            try {
                // ── LEAD ──────────────────────────────────────────────────────────────
                if (resource === 'lead') {
                    if (operation === 'getAll') {
                        const filters = this.getNodeParameter('filters', i, {});
                        const qs = Object.fromEntries(Object.entries(filters).filter(([, v]) => v !== '' && v !== 0));
                        ...
                        const mapperData = this.getNodeParameter('leadFields', i, { mappingMode: 'defineBelow', value: null });
                        const dynamicFields = (_a = mapperData.value) !== null && _a !== void 0 ? _a : {};
                        requestOptions = {
                            ...requestOptions,
                            method: 'POST',
                            url: `${baseUrl}/leads`,
                            body: { name, ...dynamicFields },
                        };
                    }
                    else if (operation === 'update') {
                        const id = this.getNodeParameter('leadId', i);
                        const mapperData = this.getNodeParameter('leadUpdateFields', i, { mappingMode: 'defineBelow', value: null });
                        const body = ((_b = mapperData.value) !== null && _b !== void 0 ? _b : {});
                    ...
                    if (operation === 'getAll') {
                        ...
                        const id = this.getNodeParameter('clientId', i);
                        const body = this.getNodeParameter('clientUpdateFields', i, {});
                        const clean = Object.fromEntries(Object.entries(body).filter(([, v]) => v !== '' && v !== null));
                        ...
                        const opts = this.getNodeParameter('checkinClientOptions', i, {});
                        const qs = { client_id: clientId };
                        if (opts.status)
                            qs.status = opts.status;
                        if (opts.from_date)
                            qs.from_date = opts.from_date;
                        if (opts.to_date)
                            qs.to_date = opts.to_date;
                        if (opts.page)
                            qs.page = opts.page;
                        if (opts.per_page)
                            qs.per_page = opts.per_page;
                    ...
                    if (operation === 'getAll') {
                        ...
                        const clientId = this.getNodeParameter('checkinClientId', i);
                        const opts = this.getNodeParameter('checkinClientOptions', i, {});
                        const qs = { client_id: clientId };
                        if (opts.status)
                            qs.status = opts.status;
                    ...
                    if (operation === 'getAll') {
                        ...
                        const desc = this.getNodeParameter('webhookDescription', i, '');
                        requestOptions = {
                            ...requestOptions,
                            method: 'POST',
                            url: `${baseUrl}/webhooks`,
                            body: { url, events, description: desc || undefined },
                        };
                    }
                    else if (operation === 'delete') {
                ...
                const result = (_c = parsed.data) !== null && _c !== void 0 ? _c : parsed;
...
exports.Brixfit = Brixfit;
// ── Shared helper: fetch lead field definitions from Brixfit API ─────────────
async function loadLeadFieldsFromApi(context, includeNameField) {
    var _a, _b, _c;
    ...
    try {
        ...
        rawFields = (_a = response.data) !== null && _a !== void 0 ? _a : [];
    }
    catch {
    ...
    for (const f of rawFields) {
        ...
        if (['email', 'phone', 'status', 'name'].includes(f.field_key))
            continue;
        fields.push({
            id: f.field_key,
            displayName: f.label,
            required: (_b = f.is_required) !== null && _b !== void 0 ? _b : false,
            defaultMatch: false,
            display: true,
            type: (_c = typeMap[f.field_type]) !== null && _c !== void 0 ? _c : 'string',
            canBeUsedToMatch: false,
        });
    }
    return { fields };
}
//# sourceMappingURL=Brixfit.node.js.map